...
Keep existing opt-in assertions and add source-level guards that SGR parsing is behind the same opt-in predicate. This test file owns fullscreen-mode mouse policy and opt-in app-wheel source evidence:
...
assert.match(source, /const mouseTrackingEnabled = isMouseTrackingEnabled\(ctx\)/);
assert.match(source, /mouseTrackingEnabled && isMouseSequence\(incoming\)/);
assert.match(source, /process\.stdout\.write\('\\x1b\[\?1000h\\x1b\[\?1006h'\)/);
```

Mandatory source-level coverage for this cycle:

- `tests/unit/tui-copy-friendly-mouse.test.ts` reads `bin/commands/tui/fullscreen-mode.ts` only.
...
- Assert `Screen.enableMouse()` still emits `\x1b[?1000h\x1b[?1006h` for app-wheel mode.
