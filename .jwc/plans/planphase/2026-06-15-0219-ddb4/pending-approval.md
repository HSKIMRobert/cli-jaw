...
6. Regression-only verification tests (no required source edits unless existing assertions conflict):
...
Declare `const mouseTrackingEnabled = isMouseTrackingEnabled(ctx);` exactly once per stdin `data` dispatch, immediately before SGR parsing. Do not redeclare it inside the parsed branch or hoist it outside the handler where `ctx` settings could drift.
...
Call-site diff reuses the single declaration from §2; do **not** add a second `const mouseTrackingEnabled` in this block:
...
Keep existing opt-in assertions and add source-level guards that SGR parsing is behind the same opt-in predicate. This test file owns fullscreen-mode mouse policy and opt-in app-wheel source evidence. It must read both `fullscreen-mode.ts` and `src/cli/tui/render/frame.ts`:
...
- `tests/unit/tui-copy-friendly-mouse.test.ts` reads `bin/commands/tui/fullscreen-mode.ts` and `src/cli/tui/render/frame.ts`.
...
- Assert opt-in startup still calls `screen.enableMouse()` through `isMouseTrackingEnabled(ctx)`.
- Assert `Screen.enableMouse()` still emits `\x1b[?1000h\x1b[?1006h` for app-wheel mode by checking `frameSource`.
...
Also update the existing launch startup snippet/assertion that currently embeds `ctx.tuiConfig['mouseTracking'] === true`; it must assert the helper-gated form instead:

```ts
assert.match(fullscreenSource, /if \(isMouseTrackingEnabled\(ctx\)\) screen\.enableMouse\(\)/);
```

This is mandatory evidence for the `bin/commands/chat.ts` diff.

Do not duplicate this assertion in `tests/unit/tui-copy-friendly-mouse.test.ts`; ownership of chat default evidence belongs here.

### 6. Tests for opt-in app wheel mode

Add mandatory source-level coverage in `tests/unit/tui-copy-friendly-mouse.test.ts` for opt-in app-wheel mode:

- Startup gate uses `if (isMouseTrackingEnabled(ctx)) screen.enableMouse();`.
- `Screen.enableMouse()` emits `\x1b[?1000h\x1b[?1006h`.
- SGR wheel reports are parsed only after `mouseTrackingEnabled && isMouseSequence(incoming)`.
- Wheel up/down preserve the viewport scroll path:

```ts
viewport.scrollBy(ev.kind === 'wheel-up' ? -3 : 3, h);
```

Runtime raw-stdin coverage is deferred unless an existing harness supports it without broad infrastructure work. Do not redesign the opt-in press `disableMouse()` + 2s `enableMouse()` escape hatch in this cycle; document it as an opt-in app-wheel selection tradeoff only. The checkable documentation sentence lives in the `tests/unit/tui-copy-friendly-mouse.test.ts` test title/body: app-wheel mode preserves wheel scrolling but native drag selection remains a documented tradeoff.
...
- `tests/unit/tui-screen-lifecycle.test.ts` is regression-only verification for screen cleanup/mouse disable lifecycle.
- `tests/unit/tui-fullscreen-frame-harness.test.ts` is regression-only verification for committed-row reachability and manual scroll behavior.
...
7. Native drag selection and terminal scrollback are manually verified in Ghostty as the behavioral acceptance for “both work”; automated source tests prove the necessary precondition (default mode does not request mouse reporting).
...
## A-Stage Audit Synthesis a-r1

Audit reports:

- Planner lens: `.jwc/plans/planphase/2026-06-15-0219-ddb4/audit-planner-a-r1.md`
- Architect lens: `.jwc/plans/planphase/2026-06-15-0219-ddb4/audit-architect-a-r1.md`
- Synthesis packet: `devlog/_plan/260614_interactive_mode_full_port/334_cycle45_5_audit_synthesis_a_r1.md`

Accepted findings and plan changes for revision `a-r2`:

- PLANNER-A1 / ARCH-A1 accepted: `tests/unit/tui-copy-friendly-mouse.test.ts` now owns mandatory opt-in startup and DECSET source assertions.
- PLANNER-A2 accepted: `tests/unit/tui-chat-launch-scope.test.ts` is the sole owner of the `chat.ts` default assertion.
- PLANNER-A3 accepted: opt-in press/timer selection tradeoff is documented in `tests/unit/tui-copy-friendly-mouse.test.ts` test naming/body plus plan risks; no user-facing docs are added in this cycle.
- PLANNER-A4 accepted: manual Ghostty check is explicit behavioral acceptance for native drag selection and terminal scrollback.
- PLANNER-A5 accepted: `mouseTrackingEnabled` is declared exactly once per stdin `data` dispatch.
- PLANNER-A6 accepted: lifecycle/frame harness tests are labeled regression-only verification.
- PLANNER-A7 accepted: F7 is addressed as source-only automated evidence plus manual Ghostty behavior evidence.
- ARCH-A2 accepted: `tests/unit/tui-chat-launch-scope.test.ts` startup snippet must align with `isMouseTrackingEnabled(ctx)`.
- ARCH-A3 accepted: implementation must update production and source-contract tests atomically.
...
| --- | --- |
| Default fullscreen starts native/copy-friendly | `chat.ts` no longer defaults `mouseTracking: true`; `tests/unit/tui-chat-launch-scope.test.ts` asserts that default; startup only calls `screen.enableMouse()` through `isMouseTrackingEnabled(ctx)`. |
...
| Opt-in app wheel mode still works | `tests/unit/tui-copy-friendly-mouse.test.ts` asserts `if (isMouseTrackingEnabled(ctx)) screen.enableMouse()`, `Screen.enableMouse()` emits `\x1b[?1000h\x1b[?1006h`, and wheel events call `viewport.scrollBy(...)`. |
| Native drag selection and terminal scrollback work without config | Manual Ghostty check verifies drag selection and native scrollback; automated source tests verify the prerequisite that default mode does not request mouse reporting. |
| Opt-in press timer tradeoff remains scoped | `tests/unit/tui-copy-friendly-mouse.test.ts` documents app-wheel mode as wheel-friendly with native drag selection tradeoff; no press timer redesign occurs in this cycle. |
