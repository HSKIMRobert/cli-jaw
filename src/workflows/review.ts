...
export function buildReviewSteerPrompt(flags: ReviewFlags, target: ReviewTargetContext): string {
    const lines = [
        `[System] User invoked /review. Perform a one-shot project-dir code review of uncommitted changes.`,
        '',
        ...
        '2. Run `git diff --stat HEAD`, `git diff HEAD`, and `git ls-files --others --exclude-standard` in the Project root. If both diff and untracked files are empty, report "No uncommitted changes to review." and still save the Markdown report.',
        ...
        '5. Review the diff and untracked files following the skill methodology in order:',
    ...
    if (flags.fix) {
        lines.push(
            ...
            '10. Apply fixes only inside the validated Project root. Do not edit JAW_HOME unless it is the validated git root, which should normally be rejected by the contract above.',
