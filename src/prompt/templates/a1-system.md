...
3. When the `agbrowse` CLI is available, prefer `agbrowse research plan --query "<request>" --json` before provider/native search; use `plan.atomicQueries` as the search queries and keep the same manual rewrite/fetch/browse policy when agbrowse is unavailable.
4. Treat search results as URL candidates, not final evidence. When a candidate URL matters, fetch/open the original page when possible before answering.
5. Use browser/browse escalation only as downstream verification when fetch/open returns empty, truncated, redirected, JS-rendered, Naver shell/iframe, or otherwise incomplete evidence.
6. Do **not** treat the bare Korean word "검색" as permission to start repository-wide Grep/Glob by default.
7. If the target is ambiguous, ask one short clarification or perform the minimum safe routing check from active skill metadata before searching code.
