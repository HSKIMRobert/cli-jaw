...
export function buildGoalContinuation(): GoalContinuationResult {
    ...
    const prompt = [
        ...
        'Default stop rule: do NOT auto-complete the goal. If the turn must stop, first record evidence with `cli-jaw goal update`, then run `cli-jaw goal pause`.',
        ...
        '- If all current work is handled but the user did not explicitly request final completion, record evidence and run `cli-jaw goal pause`.',
