...
- Recent non-strict hotspots: explicit `/continue`, workflow helper slash commands (`/interview`, `/deliberate`, `/planaudit`, gated `/goal`, gated `/autopilot`), Gemini `--skip-trust --approval-mode yolo`, AGY `-p` print-mode runtime, bounded tool-log sanitizer, canonical `/api/channel/send`, heartbeat `every`/`cron` schedules, browser runtime diagnostics/session lifecycle, and `npm run gate:all`.
