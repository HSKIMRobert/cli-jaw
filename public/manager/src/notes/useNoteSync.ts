...
export function useNoteSync(options: UseNoteSyncOptions): void {
    ...
    useEffect(() => {
        ...
        function connect(): void {
            ...
            const url = `${protocol}//${window.location.host}/api/dashboard/notes/ws`;
            ...
            ws.onmessage = (event) => {
                try {
                    ...
                    if (data.type === 'file-changed') {
                        callbacksRef.current.onFileChanged?.(data.path);
                    } else if (data.type === 'tree-changed') {
                        callbacksRef.current.onTreeChanged?.();
                    }
