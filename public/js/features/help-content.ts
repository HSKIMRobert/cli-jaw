...
export const HELP_TOPICS: Record<HelpTopicId, HelpTopic> = {
    ...
    employees: topic('employees', 3, 3, 2, 4),
    skills: topic('skills', 2, 1, 1, 4),
    ...
    mcp: topic('mcp', 2, 1, 1, 4),
    memory: topic('memory', 2, 1, 2, 4),
    stt: topic('stt', 2, 1, 1),
    promptTemplates: topic('promptTemplates', 2, 1, 1),
    chatInput: topic('chatInput', 3, 2, 2),
    orchestration: topic('orchestration', 3, 2, 2, 8),
    attachments: topic('attachments', 3, 2, 2),
    diagrams: topic('diagrams', 3, 2, 2),
    keyboardShortcuts: topic('keyboardShortcuts', 3, 2, 2, 3, 2),
    session: topic('session', 3, 1, 2, 8),
    cliTools: topic('cliTools', 3, 1, 2, 6),
    agentControl: topic('agentControl', 3, 1, 2, 6),
    goals: topic('goals', 3, 1, 2, 10),
};
