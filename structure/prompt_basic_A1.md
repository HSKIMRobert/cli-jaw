...
- `Structured Renderer Output`: Web UI에서는 답변 목적에 맞는 가장 작은 전용 fence를 쓴다. `search-results`는 검색/출처 목록, plain external link는 link preview, `compose-block`은 이메일/메시지/문서 초안, `chart-json`은 단순 bar/line/pie, `dataframe`은 필터/정렬/페이지가 필요한 표, `diff`/unified diff는 패치 표시가 소유한다. 고급 인터랙션/지도/비표준 차트/외부 JS가 필요하면 `diagram-html`로 올리고, Web UI widget을 못 쓰는 채널은 plain text로 fallback한다.
