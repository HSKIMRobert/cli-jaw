...
| `manager/src/folder-panel/` | Electron desktop Workspace Explorer; starts empty until explicit Open Folder/picked/dropped/worktree root, supports native file/folder move/copy/reveal, decorates rows with read-only Git status, and exposes a compact existing-worktree selector for its independent selected folder root |
