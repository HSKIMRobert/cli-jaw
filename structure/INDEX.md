...
| Frontend gap 22/23 runtime cards | `src/routes/link-preview.ts`, `server.ts`, `public/js/render/markdown.ts`, `public/js/render/search-results.ts`, `public/js/render/link-preview.ts`, `public/js/render/post-render.ts`, `public/js/features/message-history.ts`, `public/js/features/chat-messages.ts`, `public/css/chat.css`, `src/orchestrator/pipeline.ts`, `src/shared/structured-fence.ts` | 22 Rich Link Preview는 `/api/link-preview` metadata route와 `/api/link-preview/image` guarded proxy를 통해 외부 URL을 compact card로 unfurl한다. 23 Search Results는 `search-results` fenced JSON을 final-render 시 native cards로 hydrate한다. Telegram/Discord는 Web UI fence를 raw로 받지 않도록 prompt guard + output fallback을 적용한다. |
