...
| [server_api.md](server_api.md) | `server.ts` 글루 + `src/routes/`/mounted sub-router API 195 handlers + SSE `/api/events` + 47 WebSocket broadcast events | REST, SSE, WebSocket, 라우트 |
...
| Interview structured tracker + multi-question | `src/orchestrator/state-machine.ts`, `src/orchestrator/pipeline.ts`, `public/js/ws.ts`, `public/index.html`, `public/css/orc-state.css`, `src/prompt/templates/orchestration.md` | Interview(I)는 라운드당 1~3개 질문 + 응답마다 `<interview_tracker>` known/unknown 추출 → `OrcContext.interview`(known/unknown/round) → `orc_state` WS payload → `#interviewPanel`(`renderInterviewPanel`) 실시간 렌더. commands/agent_spawn/frontend docs reflect this. |
...
| Route extraction after SSE cycle | `server.ts`, `src/routes/static.ts`, `src/routes/messages.ts`, `src/routes/system.ts`, `src/routes/agent-control.ts`, `src/routes/command.ts`, `src/routes/instance.ts`, `src/routes/chat-sessions.ts`, `src/routes/task.ts`, `src/routes/events.ts` | `server.ts`는 654L glue로 축소되었고, base/session/message/task/event surfaces가 route modules로 분리되었다. `server_api.md`와 `str_func.md` counts must track 195 handlers including `/`. |
| Electron self-contained sidecar release | `scripts/bundle-sidecar.sh`, `electron/electron-builder.yml`, `.github/workflows/desktop-release.yml`, `electron/src/main/lib/jaw-spawn.ts`, `electron/src/main/lib/install-cli.ts` | Packaged desktop builds include a Node.js 22.16 sidecar under `extraResources/server`, prefer bundled `bin/jaw` before global `jaw`, expose first-launch/tray CLI install flow, and build macOS arm64 DMG/ZIP, Windows x64 NSIS/ZIP, and Linux AppImage from GitHub Actions release/manual dispatch. |
| Manager InstanceRegistry + event diff | `src/manager/instance-registry.ts`, `src/manager/server.ts`, `public/js/event-channel.ts` | Manager scans are served from a cached `InstanceRegistry` with diff events, reducing full-scan churn and aligning refresh-state recovery tests with the SSE channel callbacks. |
