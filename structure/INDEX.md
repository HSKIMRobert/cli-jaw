...
| [agent_spawn.md](agent_spawn.md) | CLI spawn + ACP 분기 + AGY print-mode + Claude Interactive helper + Gemini full-access flags + Grok streaming-json runtime + 오케스트레이션 | spawn, ACP, AGY, claude-e, Gemini, Grok, 멀티에이전트 |
...
| [commands.md](commands.md) | 24개 슬래시 커맨드 + root CLI 19개 user-facing command + 10개 CLI registry runtime + `browser web-ai` + explicit `/continue` note | 커맨드, 디스패처, 레지스트리 |
...
| AGY runtime | `src/types/cli-engine.ts`, `src/cli/registry.ts`, `src/cli/readiness.ts`, `src/agent/args.ts`, `src/agent/agy-runtime.ts`, `src/agent/spawn.ts`, `src/agent/spawn-env.ts`, `src/routes/settings.ts`, `public/index.html`, `public/js/constants.ts`, `public/js/features/settings-cli-status.ts`, `public/manager/src/settings/pages/components/agent/agent-meta.ts`, `bin/commands/init.ts`, `bin/commands/tui/types.ts` | `agy` is a top-level registry runtime, not an `ai-e` provider. It uses `agy -p` print mode with `--print-timeout 10m`, optional `--dangerously-skip-permissions`, repeated `--add-dir`, plain-text stdout, default `NO_COLOR=1`, run-time auth checking, and no native resume/model/effort flags. `/api/cli-registry`, `/api/cli-status`, and `/api/quota` include AGY; quota is status-only because AGY does not expose quota windows. Antigravity MCP sync remains a separate config target. |
| Claude E runtime wrapper | `native/jaw-claude-i/`, `src/agent/claude-e-runtime.ts`, `src/agent/args.ts`, `src/agent/events.ts`, `src/core/config.ts`, `src/cli/readiness.ts`, `bin/commands/doctor.ts` | `claude-e` is an experimental registry runtime backed by the Claude E helper surface; helper discovery honors `CLAUDE_E_BIN`, bundled npm `claude-e`, PATH `claude-e`, compatibility `claude-exec`, then legacy `jaw-claude-i` / `claude-i` fallbacks. Runtime events and session bucket remain legacy `agent:claude-i:*` / `claude-i`, and `npm run build:claude-exec` / `npm run test:claude-exec` remain native fallback build/test aliases. |
...
*마지막 갱신: 2026-05-21 (`server.ts` 847L, `src/routes/` 18 files, `src/routes/browser.ts` 475L, `src/manager/` 69 TS/TSX files / 10741L, `src/browser/web-ai/` 67 TS files / 12415L, `src/browser/adaptive-fetch/` 18 TS mirror files, `src/agent/` 35 TS files, `native/jaw-claude-i/` 11 Rust source files / 1934L, `bin/commands/` 22 top-level ts files + `tui/` 7 helper, `public/js/` 75 .ts, tests 400 .test.ts / unit 371 .test.ts 기준)*
