...
- `src/agent/args.ts` + `src/agent/spawn.ts` + `src/agent/spawn-env.ts` + `src/cli/registry.ts` + `src/cli/readiness.ts`: AGY is a top-level `agy` runtime, not an `ai-e` provider. It uses `agy -p` print mode, plain-text stdout, `NO_COLOR=1`, run-time auth checking, and no native resume/model/effort flags.
