...
| `quota.ts` | `/api/quota` helper readers imported by `settings.ts` (Claude/Codex/Gemini/Copilot usage + Grok auth/status-only; AGY is returned by `settings.ts` as status-only until a quota/auth API exists) |
