...
> slash registry는 29개 커맨드, 4개 실행 인터페이스. root CLI는 `bin/cli-jaw.ts` + `bin/commands/*.ts` 기준 19개 user-facing command이며, helper까지 포함한 `bin/commands/*.ts` top-level 파일은 22개다. `browser web-ai`는 `browser-web-ai.ts`, `dashboard memory`는 `dashboard-memory.ts`, dispatch unwrap 보조는 `dispatch-helpers.ts`로 분리되어 있다. visible 기준 CLI 27 / Web 25 / Telegram 25 / Discord 25. `cmdline` capability는 contract 전용이며 11개가 보인다.
...
| --- | ---: | --- |
| `cli` | 27 | `file` hidden, `steer` 미지원 |
| `web` | 25 | `commands`, `quit`, `file`, `ide` 미지원 |
| `telegram` | 25 | remote-safe command set |
| `discord` | 25 | remote-safe command set |
...
- `workflow`: `interview`, `deliberate`, `planaudit`, `goal`
...
그 결과 `cmdline` visible command는 11개다. `file`은 slash registry에서는 hidden이지만 `cmdline` capability hidden set에는 없어서 contract visible 쪽에 남고, `interview`/`deliberate`/`planaudit`/`goal`은 workflow category라 hidden 처리된다.
